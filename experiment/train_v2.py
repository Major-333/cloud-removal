...
class Trainer(object):
    ...
    def _setup(self) -> None:
        self.train_loader, self.val_loader = self.init_helper.init_train_test_dsen2cr_dataloaders()
        ...
        try:
            ...
            for epoch in range(1, self.config['epochs']):
                ...
                if index % wandb.config['validate_freq'] == 0 and rank == 0:
                    ...
                    logs = logs | metric
                wandb.log(logs)
                save_checkpoints(self.model, epoch, start_time=START_TIME, suffix=f'rank{rank}')
                self.scheduler.step()   
