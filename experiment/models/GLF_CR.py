...
class RDN_residual_CR(nn.Module):
    def __init__(self, input_size):
        ...
        self.UPNet = nn.Sequential(*[
            ...
            nn.PixelShuffle(2),
            nn.Conv2d(64, 13, 3, padding=1, stride=1)
        ])

    def forward(self, cloudy_data):
        SAR = cloudy_data[:, :2, :, :]
        cloudy_data = cloudy_data[:, 2:, :, :]
        B = cloudy_data.shape[0]
        if not self.training:
            cloudy_data = torch.cat(
                (cloudy_data[:, :, :128, :128], #左上
                 cloudy_data[:, :, 128:, :128], #左下
                 cloudy_data[:, :, 128:, 128:], #右下
                 cloudy_data[:, :, :128, 128:]), #右上
                dim=0)
            SAR = torch.cat(
                (SAR[:, :, :128, :128],
                 SAR[:, :, 128:, :128],
        ...
        if not self.training:
            pred_CloudFree_data_left_up = pred_CloudFree_data[:B, :, :, :]
            pred_CloudFree_data_left_bottom = pred_CloudFree_data[B:2*B, :, :, :]
            pred_CloudFree_data_right_bottom = pred_CloudFree_data[2*B:3*B, :, :, :]
            pred_CloudFree_data_right_up = pred_CloudFree_data[3*B:, :, :, :]
            pred_CloudFree_data_up = torch.cat((pred_CloudFree_data_left_up, pred_CloudFree_data_right_up), dim=3)
            pred_CloudFree_data_bottom = torch.cat((pred_CloudFree_data_left_bottom, pred_CloudFree_data_right_bottom), dim=3)
            pred_CloudFree_data= torch.cat((pred_CloudFree_data_up, pred_CloudFree_data_bottom), dim=2)
...
if __name__ == "__main__":
    ...
    model = RDN_residual_CR(128).cuda()

    planet_cloudy = torch.rand(1, 15, 128, 128).cuda()

    pred_planet_cloudfree = model(planet_cloudy)
    ...
    contributing_parameters = set(get_contributing_params(pred_planet_cloudfree))
