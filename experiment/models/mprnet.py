...
class MPRNet(nn.Module):
    def __init__(self, n_feat=80, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=8, kernel_size=3, reduction=4, bias=False):
        super(MPRNet, self).__init__()

        act=nn.PReLU()
        self.shallow_feat1 = nn.Sequential(conv(15, n_feat, kernel_size, bias=bias), CAB(n_feat,kernel_size, reduction, bias=bias, act=act))
        self.shallow_feat2 = nn.Sequential(conv(15, n_feat, kernel_size, bias=bias), CAB(n_feat,kernel_size, reduction, bias=bias, act=act))
        self.shallow_feat3 = nn.Sequential(conv(15, n_feat, kernel_size, bias=bias), CAB(n_feat,kernel_size, reduction, bias=bias, act=act))
        ...
        self.tail     = conv(n_feat+scale_orsnetfeats, 13, kernel_size, bias=bias)
    ...
    def forward(self, x3_img):
        ...
        x2top_samfeats, stage1_img_top = self.sam12(res1_top[0], x2top_img[:, 2:, :, :])
        x2bot_samfeats, stage1_img_bot = self.sam12(res1_bot[0], x2bot_img[:, 2:, :, :])
        ...
        ## Apply SAM
        x3_samfeats, stage2_img = self.sam23(res2[0], x3_img[:, 2:, :, :])
        ...
        return [stage3_img+x3_img[:, 2:, :, :], stage2_img, stage1_img]
