...
def build_distributed_loaders_with_rois(dataset_path: str, batch_size: int, file_extension: str, rois: List[Roi], debug: bool=False) -> Tuple[DataLoader, DataLoader, DataLoader]:
    dataset = SEN12MSCRDataset(dataset_path, file_extension, rois=rois, debug=debug)
    data_sampler = DistributedSampler(dataset)
    return DataLoader(dataset, batch_size=batch_size, num_workers=4, pin_memory=True, sampler=data_sampler)


def build_loaders_with_rois(dataset_path: str, batch_size: int, file_extension: str, rois: List[Roi], debug: bool=False) -> Tuple[DataLoader, DataLoader, DataLoader]:
    dataset = SEN12MSCRDataset(dataset_path, file_extension, rois=rois, debug=debug)
    return DataLoader(dataset, batch_size=batch_size, num_workers=4, pin_memory=True)


def build_loaders(dataset_path: str, batch_size: int, file_extension: str, debug: bool=False) -> Tuple[DataLoader, DataLoader, DataLoader]:
    """ Returns a tuple of dataloader, which are train_loader, val_loader, test_loader
    """
    dataset = SEN12MSCRDataset(dataset_path, file_extension, debug=debug)
...
def build_distributed_loaders(dataset_path: str, batch_size: int, file_extension: str, debug: bool=False) -> Tuple[DataLoader, DataLoader, DataLoader]:
    dataset = SEN12MSCRDataset(dataset_path, file_extension, debug=debug)
    n_train = int(len(dataset) * 0.6)
    n_test = int(len(dataset) * 0.2)
    n_val = len(dataset) - n_train - n_test
