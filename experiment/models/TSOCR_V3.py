...
class CrossAttention(nn.Module):
    ...
    def forward(self, sar, ms):
        ...
        sar2ms_cross_attn = (sar_q @ ms_k.transpose(-2, -1)) * self.ms_temperature
        sar2ms_cross_attn = sar2ms_cross_attn.softmax(dim=-1)

        sar2ms_cross_attn_out = (sar2ms_cross_attn @ sar_v)

        sar2ms_cross_attn_out = rearrange(sar2ms_cross_attn_out, 'b head c (h w) -> b (head c) h w', head=self.num_heads, h=h, w=w)

        ms2sar_cross_attn = (ms_q @ sar_k.transpose(-2, -1)) * self.ms_temperature
        ms2sar_cross_attn = ms2sar_cross_attn.softmax(dim=-1)

        ms2sar_cross_attn_out = (ms2sar_cross_attn @ sar_v)

        ms2sar_cross_attn_out = rearrange(ms2sar_cross_attn_out, 'b head c (h w) -> b (head c) h w', head=self.num_heads, h=h, w=w)

        sar_out = self.sar_project_out(sar_self_attn_out + ms2sar_cross_attn_out)
        ms_out = self.ms_project_out(ms_self_attn_out + sar2ms_cross_attn_out)
