...
class Preprocess(object):
    ...
    def _crop_triplets(self, datas: Tuple[array, array, array]) -> Tuple[array, array, array]:
        ...
        cropped_images = [np.zeros((n, sar_channel, h//2, w//2))] + [np.zeros((n, c, h//2, w//2)) for _ in range(2)]
        # TODO: optimize this for-loop
        for image_idx in range(n):
            input = [data[image_idx, :, :, :] for data in datas]  # input: List len(input) == 3
